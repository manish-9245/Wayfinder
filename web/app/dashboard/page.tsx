  ...
  if (me.err)
    return (
      <Card className="mx-auto mt-10 max-w-2xl">
        <CardHeader>
          <p className="eyebrow">Wayfinder — your account</p>
          <CardTitle className="mt-2 font-tsj-display">Dashboard unavailable</CardTitle>
    ...
    <RequireAuth>
      <>
        <div className="mb-8 mt-6">
          <p className="eyebrow">Wayfinder — your account</p>
          <h1 className="mt-2 font-tsj-display text-4xl font-bold tracking-tight md:text-5xl">
            Dashboard{" "}
            <span className="font-tsj-mono text-lg font-medium normal-case tracking-normal text-muted-foreground">
        ...
        {t && (
          <section aria-label="Usage" className="mb-10">
            <SectionNum n="01" title="Usage · 30 days" />
            <Kpis items={[
        ...
        <section aria-label="Monthly quota" className="mb-10">
          <SectionNum n="02" title="Monthly quota" />
          <Card>
              ...
              {quota == null ? (
                ...
                <>
                  <div className="flex items-baseline justify-between gap-3 text-[13px]">
                    <code className="rounded-md border bg-background px-2 py-0.5 font-tsj-mono text-xs tabular-nums">{quotaUsed} / {quota}</code>
                    <span className="font-tsj-mono tabular-nums text-muted-foreground">
        ...
        <section aria-label="API keys" className="mb-10">
          <SectionNum n="03" title="API keys" />
          <p className="mb-3 max-w-[68ch] font-tsj-mono text-xs text-muted-foreground">
            Service keys start with <code className="rounded border bg-background px-1.5 py-px font-tsj-mono text-xs">wf_</code>.
            The raw secret is shown once — store it in an env var. Creating a key also enables it in this
            browser&apos;s console.
          </p>
          {freshKey && (
            <div className="mb-3 flex flex-wrap items-center gap-2.5 rounded-md border border-dashed border-info bg-background px-3.5 py-3 font-tsj-mono text-[13px]" role="status">
          ...
          {(keys.data?.length ?? 0) > 0 && (
            <div className="hairline-t mt-4 overflow-x-auto">
              <Table>
                <TableHeader>
                  <TableRow>
                    <TableHead className="font-tsj-mono text-[11px] uppercase tracking-[0.14em]">Name</TableHead>
                    <TableHead className="font-tsj-mono text-[11px] uppercase tracking-[0.14em]">Prefix</TableHead>
                    <TableHead className="font-tsj-mono text-[11px] uppercase tracking-[0.14em]">Calls</TableHead>
                    <TableHead className="font-tsj-mono text-[11px] uppercase tracking-[0.14em]">Last used</TableHead>
        ...
        <section aria-label="Request logs">
          <SectionNum n="04" title="Request logs" />
